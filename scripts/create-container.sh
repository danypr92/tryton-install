...
# Create `tryton` group with same `gid` of project directory
sudo lxc-attach -n "$NAME" -- /usr/sbin/groupadd --gid "$project_gid" tryton

# Create `tryton` user with same `uid` and `gid` of project directory
sudo lxc-attach -n "$NAME" -- /usr/sbin/useradd --uid "$project_uid" --gid "$project_gid" --create-home --shell /bin/bash tryton

# Add system user's SSH public key to `tryton` user
echo "Copying system user's SSH public key to 'tryton' user in container"
sudo lxc-attach -n "$NAME" -- /bin/bash -c "/bin/mkdir -p /home/tryton/.ssh && echo $ssh_key > /home/tryton/.ssh/authorized_keys"
